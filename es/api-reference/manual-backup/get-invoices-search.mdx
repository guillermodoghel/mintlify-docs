---
title: 'GET /invoices - Buscar Facturas'
description: 'Documentación para buscar y filtrar facturas con parámetros de consulta'
---

# 🔍 GET /invoices - Buscar Facturas

Este endpoint te permite buscar y filtrar facturas usando diferentes parámetros de consulta.

## URL Base

```
https://facture.ar/api/invoices
```

## Autenticación

Todas las peticiones requieren autenticación usando:

- **Sesión Web**: Para aplicaciones basadas en navegador
- **API Key**: Para integraciones servidor a servidor

Incluye tu API key en el header `x-api-key`:

```bash
x-api-key: tu-api-key-aqui
```

## Parámetros de Consulta

| Parámetro     | Tipo   | Requerido | Descripción                                      | Ejemplo                                  |
| ------------- | ------ | --------- | ------------------------------------------------ | ---------------------------------------- |
| `cuitId`      | string | ❌        | Filtrar por ID de CUIT                           | `"123e4567-e89b-12d3-a456-426614174000"` |
| `status`      | string | ❌        | Filtrar por estado                               | `"success"`                              |
| `invoiceType` | number | ❌        | Filtrar por tipo de factura                      | `11`                                     |
| `dateFrom`    | string | ❌        | Fecha desde (YYYY-MM-DD)                         | `"2024-01-01"`                           |
| `dateTo`      | string | ❌        | Fecha hasta (YYYY-MM-DD)                         | `"2024-01-31"`                           |
| `amountMin`   | number | ❌        | Monto mínimo                                     | `100.0`                                  |
| `amountMax`   | number | ❌        | Monto máximo                                     | `5000.0`                                 |
| `clientName`  | string | ❌        | Buscar por nombre de cliente                     | `"Cliente S.A."`                         |
| `page`        | number | ❌        | Número de página (por defecto: 1)                | `1`                                      |
| `limit`       | number | ❌        | Elementos por página (por defecto: 20, máx: 100) | `50`                                     |
| `sortBy`      | string | ❌        | Campo para ordenar                               | `"createdAt"`                            |
| `sortOrder`   | string | ❌        | Orden (asc/desc, por defecto: desc)              | `"desc"`                                 |

## Respuesta

### Éxito (200)

```json
{
  "data": [
    {
      "id": "123e4567-e89b-12d3-a456-426614174000",
      "cuitId": "456e7890-e89b-12d3-a456-426614174001",
      "amount": 1000.0,
      "description": "Venta de servicios",
      "docTipo": 99,
      "docNro": 0,
      "invoiceType": 11,
      "concepto": 2,
      "ptoVta": 1,
      "clientName": "Cliente S.A.",
      "cbteFch": "20240101",
      "status": "success",
      "afipData": {
        "cae": "12345678901234",
        "caeFchVto": "20240115",
        "voucherNumber": 1,
        "ptoVta": 1,
        "invoiceType": 11
      },
      "createdAt": "2024-01-01T10:00:00Z",
      "updatedAt": "2024-01-01T10:05:00Z"
    }
  ],
  "pagination": {
    "page": 1,
    "limit": 20,
    "total": 150,
    "totalPages": 8,
    "hasNext": true,
    "hasPrev": false
  },
  "filters": {
    "cuitId": "456e7890-e89b-12d3-a456-426614174001",
    "status": "success",
    "dateFrom": "2024-01-01",
    "dateTo": "2024-01-31"
  }
}
```

### Respuestas de Error

#### Error de Validación (400)

```json
{
  "error": "Validation error",
  "details": [
    {
      "field": "dateFrom",
      "message": "Invalid date format. Use YYYY-MM-DD"
    }
  ]
}
```

#### No Autorizado (401)

```json
{
  "error": "Unauthorized",
  "details": "Invalid API key or session"
}
```

#### Error del Servidor (500)

```json
{
  "error": "Internal server error",
  "details": "Failed to retrieve invoices"
}
```

## Campos de Respuesta

### Datos de Factura

| Campo         | Tipo   | Descripción                              |
| ------------- | ------ | ---------------------------------------- |
| `id`          | string | ID único de la factura                   |
| `cuitId`      | string | ID del CUIT que emitió la factura        |
| `amount`      | number | Monto total de la factura                |
| `description` | string | Descripción de la factura                |
| `docTipo`     | number | Tipo de documento del cliente            |
| `docNro`      | number | Número de documento del cliente          |
| `invoiceType` | number | Tipo de factura                          |
| `concepto`    | number | Tipo de concepto                         |
| `ptoVta`      | number | Punto de venta                           |
| `clientName`  | string | Nombre del cliente                       |
| `cbteFch`     | string | Fecha de la factura (YYYYMMDD)           |
| `status`      | string | Estado de la factura                     |
| `afipData`    | object | Datos de AFIP (solo si status = success) |
| `createdAt`   | string | Fecha de creación (ISO 8601)             |
| `updatedAt`   | string | Fecha de última actualización (ISO 8601) |

### Paginación

| Campo        | Tipo    | Descripción          |
| ------------ | ------- | -------------------- |
| `page`       | number  | Página actual        |
| `limit`      | number  | Elementos por página |
| `total`      | number  | Total de elementos   |
| `totalPages` | number  | Total de páginas     |
| `hasNext`    | boolean | Hay página siguiente |
| `hasPrev`    | boolean | Hay página anterior  |

## Estados de Factura

| Estado       | Descripción                              |
| ------------ | ---------------------------------------- |
| `pending`    | Factura en cola, esperando procesamiento |
| `processing` | Factura siendo procesada por AFIP        |
| `success`    | Factura procesada exitosamente           |
| `failed`     | Factura falló en el procesamiento        |

## Campos de Ordenamiento

| Campo        | Descripción                   |
| ------------ | ----------------------------- |
| `createdAt`  | Fecha de creación             |
| `updatedAt`  | Fecha de última actualización |
| `amount`     | Monto de la factura           |
| `cbteFch`    | Fecha de la factura           |
| `clientName` | Nombre del cliente            |

## Ejemplos de Uso

### Buscar Facturas por Estado

```bash
curl -X GET "https://facture.ar/api/invoices?status=success&limit=10" \
  -H "x-api-key: tu-api-key"
```

### Buscar Facturas por Rango de Fechas

```bash
curl -X GET "https://facture.ar/api/invoices?dateFrom=2024-01-01&dateTo=2024-01-31" \
  -H "x-api-key: tu-api-key"
```

### Buscar Facturas por CUIT y Monto

```bash
curl -X GET "https://facture.ar/api/invoices?cuitId=123e4567-e89b-12d3-a456-426614174000&amountMin=100&amountMax=1000" \
  -H "x-api-key: tu-api-key"
```

### JavaScript/Node.js

```javascript
// Buscar facturas exitosas del último mes
const response = await fetch('https://facture.ar/api/invoices?status=success&dateFrom=2024-01-01&dateTo=2024-01-31&limit=50', {
  method: 'GET',
  headers: {
    'x-api-key': 'tu-api-key',
  },
});

const result = await response.json();
console.log('Facturas:', result.data);
console.log('Total:', result.pagination.total);
```

### Python

```python
import requests

# Buscar facturas por cliente
response = requests.get(
    'https://facture.ar/api/invoices',
    headers={
        'x-api-key': 'tu-api-key'
    },
    params={
        'clientName': 'Cliente S.A.',
        'status': 'success',
        'limit': 20,
        'sortBy': 'createdAt',
        'sortOrder': 'desc'
    }
)

result = response.json()
print('Facturas:', result['data'])
print('Total:', result['pagination']['total'])
```

## ❓ Solución de Problemas

<Accordion title="Error: Formato de fecha inválido">
**Causa**: Las fechas no están en el formato correcto.

**Solución**:

1. Usa el formato YYYY-MM-DD para las fechas
2. Verifica que las fechas sean válidas
3. Asegúrate de que dateFrom sea anterior a dateTo

</Accordion>

<Accordion title="Error: Límite excedido">
**Causa**: El parámetro limit excede el máximo permitido.

**Solución**:

1. Usa un límite menor o igual a 100
2. Implementa paginación para obtener más resultados
3. Usa filtros más específicos para reducir resultados

</Accordion>

<Accordion title="Error: Estado inválido">
**Causa**: El estado especificado no es válido.

**Solución**:

1. Usa solo estados válidos: pending, processing, success, failed
2. Verifica la ortografía del estado
3. Consulta la tabla de estados disponibles
</Accordion>

## 📞 Soporte

Para soporte técnico o preguntas sobre la API:

- 📧 **Email**: support@facture.ar
- 💬 **Chat**: Disponible en la plataforma
- 📚 **Documentación**: Explora nuestros ejemplos detallados
- 🎥 **Videollamada**: Para asistencia personalizada
