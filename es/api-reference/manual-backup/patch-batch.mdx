---
title: 'PATCH /invoices/batches/{id} - Agregar Facturas a Lote'
description: 'Documentación para agregar facturas a un lote existente'
---

# 📦 PATCH /invoices/batches/{id} - Agregar Facturas a Lote

Este endpoint te permite agregar facturas adicionales a un lote existente que aún no ha sido completado.

## URL Base

```
https://facture.ar/api/invoices/batches/{id}
```

## Autenticación

Todas las peticiones requieren autenticación usando:

- **Sesión Web**: Para aplicaciones basadas en navegador
- **API Key**: Para integraciones servidor a servidor

Incluye tu API key en el header `x-api-key`:

```bash
x-api-key: tu-api-key-aqui
```

## Parámetros de Ruta

| Parámetro | Tipo   | Requerido | Descripción       | Ejemplo                                  |
| --------- | ------ | --------- | ----------------- | ---------------------------------------- |
| `id`      | string | ✅        | ID único del lote | `"123e4567-e89b-12d3-a456-426614174000"` |

## Cuerpo de la Petición

```json
[
  {
    "amount": 1000.0,
    "description": "Venta de servicios",
    "cuitId": "456e7890-e89b-12d3-a456-426614174001"
  },
  {
    "amount": 2000.0,
    "description": "Venta de productos",
    "cuitId": "456e7890-e89b-12d3-a456-426614174001"
  }
]
```

## Parámetros del Cuerpo

| Parámetro     | Tipo   | Requerido | Descripción                                       | Ejemplo                                  |
| ------------- | ------ | --------- | ------------------------------------------------- | ---------------------------------------- |
| `amount`      | number | ✅        | Monto total de la factura                         | `1000.0`                                 |
| `description` | string | ✅        | Descripción de la factura                         | `"Venta de servicios"`                   |
| `cuitId`      | string | ❌        | ID del CUIT (usa por defecto si no se especifica) | `"456e7890-e89b-12d3-a456-426614174001"` |

## Respuesta

### Éxito (200)

```json
{
  "batch": true,
  "message": "Facturas agregadas al lote exitosamente",
  "count": 2,
  "status": "processing",
  "batchId": "123e4567-e89b-12d3-a456-426614174000",
  "queueIdentifiers": ["production_cuit-id_1_11"]
}
```

### Respuestas de Error

#### Lote No Encontrado (404)

```json
{
  "error": "Lote no encontrado",
  "details": "The requested batch does not exist"
}
```

#### Lote Completado (400)

```json
{
  "error": "No se pueden agregar facturas a un lote completado",
  "details": "El lote está en estado: completed"
}
```

#### No Autorizado (403)

```json
{
  "error": "No autorizado",
  "details": "You don't have access to this batch"
}
```

#### Error de Validación (400)

```json
{
  "error": "Datos inválidos en el lote",
  "details": [
    {
      "field": "amount",
      "message": "Amount must be a positive number"
    }
  ]
}
```

#### Límite de Suscripción (403)

```json
{
  "error": "Subscription limit exceeded",
  "details": "Monthly invoice limit reached"
}
```

## Restricciones

- **Estado del Lote**: Solo se pueden agregar facturas a lotes en estado `pending` o `processing`
- **Límite de Items**: Máximo 500 facturas por lote
- **Límite de Suscripción**: Sujeto a los límites de tu plan de suscripción
- **Validación**: Todas las facturas deben pasar la validación antes de ser agregadas

## Estados de Lote Válidos

| Estado                  | Permite Agregar Facturas |
| ----------------------- | ------------------------ |
| `pending`               | ✅ Sí                    |
| `processing`            | ✅ Sí                    |
| `completed`             | ❌ No                    |
| `completed_with_errors` | ❌ No                    |
| `error`                 | ❌ No                    |

## Ejemplos de Uso

### Agregar Facturas Simples

```bash
curl -X PATCH https://facture.ar/api/invoices/batches/123e4567-e89b-12d3-a456-426614174000 \
  -H "Content-Type: application/json" \
  -H "x-api-key: tu-api-key" \
  -d '[
    {
      "amount": 1000.0,
      "description": "Venta de servicios"
    },
    {
      "amount": 2000.0,
      "description": "Venta de productos"
    }
  ]'
```

### Agregar Facturas con CUIT Específico

```bash
curl -X PATCH https://facture.ar/api/invoices/batches/123e4567-e89b-12d3-a456-426614174000 \
  -H "Content-Type: application/json" \
  -H "x-api-key: tu-api-key" \
  -d '[
    {
      "amount": 1000.0,
      "description": "Venta de servicios",
      "cuitId": "456e7890-e89b-12d3-a456-426614174001"
    }
  ]'
```

### JavaScript/Node.js

```javascript
// Agregar facturas a un lote existente
const response = await fetch('https://facture.ar/api/invoices/batches/123e4567-e89b-12d3-a456-426614174000', {
  method: 'PATCH',
  headers: {
    'Content-Type': 'application/json',
    'x-api-key': 'tu-api-key',
  },
  body: JSON.stringify([
    {
      amount: 1000.0,
      description: 'Venta de servicios',
      cuitId: '456e7890-e89b-12d3-a456-426614174001',
    },
    {
      amount: 2000.0,
      description: 'Venta de productos',
      cuitId: '456e7890-e89b-12d3-a456-426614174001',
    },
  ]),
});

const result = await response.json();
console.log('Facturas agregadas:', result);

if (result.batch) {
  console.log(`Se agregaron ${result.count} facturas al lote ${result.batchId}`);
}
```

### Python

```python
import requests

# Agregar facturas a un lote existente
response = requests.patch(
    'https://facture.ar/api/invoices/batches/123e4567-e89b-12d3-a456-426614174000',
    headers={
        'Content-Type': 'application/json',
        'x-api-key': 'tu-api-key'
    },
    json=[
        {
            'amount': 1000.0,
            'description': 'Venta de servicios',
            'cuitId': '456e7890-e89b-12d3-a456-426614174001'
        },
        {
            'amount': 2000.0,
            'description': 'Venta de productos',
            'cuitId': '456e7890-e89b-12d3-a456-426614174001'
        }
    ]
)

result = response.json()
print('Facturas agregadas:', result)

if result.get('batch'):
    print(f"Se agregaron {result['count']} facturas al lote {result['batchId']}")
```

## ❓ Solución de Problemas

<Accordion title="Error: Lote completado">
**Causa**: Intentas agregar facturas a un lote que ya fue completado.

**Solución**:

1. Verifica el estado del lote antes de intentar agregar facturas
2. Crea un nuevo lote si necesitas agregar más facturas
3. Solo se pueden agregar facturas a lotes en estado `pending` o `processing`

</Accordion>

<Accordion title="Error: Límite de suscripción excedido">
**Causa**: Has alcanzado el límite mensual de facturas de tu plan.

**Solución**:

1. Verifica tu límite de suscripción actual
2. Espera al próximo período de facturación
3. Considera actualizar tu plan si necesitas más facturas

</Accordion>

<Accordion title="Error: Datos inválidos">
**Causa**: Los datos de las facturas no pasan la validación.

**Solución**:

1. Revisa los detalles del error en la respuesta
2. Corrige los datos según los mensajes de error
3. Asegúrate de que todos los campos requeridos estén presentes
</Accordion>

## 📞 Soporte

Para soporte técnico o preguntas sobre la API:

- 📧 **Email**: support@facture.ar
- 💬 **Chat**: Disponible en la plataforma
- 📚 **Documentación**: Explora nuestros ejemplos detallados
- 🎥 **Videollamada**: Para asistencia personalizada
