...
```javascript
const response = await fetch('https://api.facturear.com/invoices/batches', {
  method: 'POST',
  headers: {
...
await fetch(`https://api.facturear.com/invoices/batches/${batchId}`, {
  method: 'PATCH',
  headers: {
    'x-api-key': 'tu_api_key',
    'Content-Type': 'application/json'
...
```javascript
const batch = await fetch(`https://api.facturear.com/invoices/batches/${batchId}`, {
...
await fetch(
  `https://api.facturear.com/invoices/batches/${batchId}/items/${itemId}/retry`,
  {
    method: 'POST',
    headers: {
      'x-api-key': 'tu_api_key',
...
```javascript
const batches = await fetch('https://api.facturear.com/invoices/batches?page=1&limit=20', {
  headers: { 'x-api-key': 'tu_api_key' }
}).then(r => r.json())
```
...
async function facturacionMensual(clientes, mes) {
  ...
  const BASE_URL = 'https://api.facturear.com'
